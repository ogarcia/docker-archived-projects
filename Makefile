...
docker-build:
	docker buildx build -t $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE) --build-arg ALPINE_VERSION=$(ALPINE_VERSION) .

docker-test:
	docker image inspect $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE)
	docker run --name $(DOCKER_IMAGE) -d -p 5900:5900 $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE)
	sleep 5
	ss -ltpn | grep 5900
